...
try {
    ...
    // Armamos la fecha límite para la consulta
    $fecha_limite = $year . "-" . $month . "-" . $day;
    
    // 4. Preparar la consulta SQL de forma segura
    // NOTA: Mantenemos el estándar de PostgreSQL usando el nombre de la tabla en minúsculas
    $consulta = "SELECT * FROM alumno WHERE fecha_modificado > :fecha";
    ...
    // 6. Devolver el JSON limpio a tu app de Android
    echo json_encode($filas);

} catch (PDOException $e) {
    // Si algo truena, devolvemos el error estructurado en JSON para no romper Android
